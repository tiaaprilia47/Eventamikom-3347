    ...
    <nav
        ...
        <div class="flex gap-3">
            @guest
                ...
                <a href="{{ route('register') }}" class="px-5 py-2.5 bg-indigo-600 text-white rounded-xl font-semibold shadow-lg shadow-indigo-200 hover:bg-indigo-700 transition">Daftar</a>
            @else
                <form action="{{ route('logout') }}" method="POST" class="inline">
                    @csrf
                    <button type="submit" class="px-5 py-2.5 rounded-xl font-semibold hover:bg-slate-200 transition">Keluar</button>
                </form>
            @endguest
        </div>
    </nav>
